...
def load_state_dict(model, state_dict, strict=False, logger=None):
    """Load state_dict into model and log incompatible keys."""
...
def _get_logger(name='tcformer', log_file=None, log_level=logging.INFO):
...
def load_checkpoint(model,
                    ...
                    logger=None,
                    revise_keys=[(r'^module\.', '')]):
    """Load checkpoint from a local file."""
    checkpoint = _load_checkpoint(filename, map_location, logger)
...
def get_root_logger(log_file=None, log_level=logging.INFO):
    return _get_logger(name='tcformer', log_file=log_file, log_level=log_level)
